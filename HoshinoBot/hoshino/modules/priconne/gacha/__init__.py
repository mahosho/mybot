...
from hoshino.util import concat_pic, pic2b64, DailyNumberLimiter
...
GACHA_DISABLE_NOTICE = '本群扭蛋功能已禁用\n如欲开启，请与维护组联系'
...
async def gacha_info(session:CommandSession):
    gid = str(session.ctx.get('group_id'))
...
async def gacha_1(session:CommandSession):
    ...
    gacha = Gacha(_group_pool[gid])
    chara, hiishi = gacha.gacha_one(gacha.up_prob, gacha.up2_prob, gacha.s3_prob, gacha.s2_prob)
...
async def gacha_300(session:CommandSession):
    ...
    s2 = len(result['s2'])
    s1 = len(result['s1'])
    res = [*(result['up']), *(result['s3'])]#这里res返回的是3星的结果集合
    for c in res:
        uset.add(c.name)
    ucollection[uid]=list(uset)
    dump_user_collection(uid,ucollection)
