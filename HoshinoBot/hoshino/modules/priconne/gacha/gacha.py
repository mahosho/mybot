...
class Gacha(object):
    ...
    def gacha_one(self, up_prob:int, up2_prob:int, s3_prob:int, s2_prob:int, s1_prob:int=None):
        '''
    ...
    #fes专用，需要自行修改
    def gacha_one_fes(self, up_prob:int, up2_prob:int, s3_prob:int, s2_prob:int, s1_prob:int=None):
        '''
        sx_prob: x星概率，要求和为1000
        up_prob: UP角色概率（从3星划出）
        up_chara: UP角色名列表
        ...
        if pick <= up_prob:#0.7
            return Chara.fromname(random.choice(self.up), 3), 100
        if pick <= up_prob + up2_prob:#fes 0.3*3+0.7
            return Chara.fromname(random.choice(self.up2),3), 50
        elif pick <= s3_prob:#5
            return Chara.fromname(random.choice(self.star3), 3), 50        
        elif pick <= s2_prob + s3_prob:
            return Chara.fromname(random.choice(self.star2), 2), 10
    ...
    def gacha_ten(self):
        ...
        for _ in range(9):    # 前9连
            c, y = self.gacha_one(up, up2, s3, s2, s1)
            result.append(c)
            hiishi += y
        c, y = self.gacha_one(up, up2, s3, s2 + s1, 0)    # 保底第10抽
        result.append(c)
        hiishi += y

        return result, hiishi

    def gacha_ten_fes(self):
        result = []
        hiishi = 0
        up = self.up_prob
        up2 = self.up2_prob
        ...
        s1 = 1000 - s3 - s2
        for _ in range(9):    # 前9连
            c, y = self.gacha_one_fes(up, up2, s3, s2, s1)
            result.append(c)
            hiishi += y
        c, y = self.gacha_one_fes(up, up2, s3, s2 + s1, 0)    # 保底第10抽
        result.append(c)
        hiishi += y
    ...
    def gacha_tenjou(self):
        ...
        for i in range(30):
            c, y = self.gacha_one(up, up2, s3, s2 + s1, 0)
            if 100 == y:
                result['up'].append(c)
    ...
    def gacha_tenjou_fes(self):
        ...
        for i in range(9 * 30):
            ...
            if 100 == y:
                ...
                pass    # should never reach here
        for i in range(30):
            c, y = self.gacha_one_fes(up, up2, s3, s2 + s1, 0)
            if 100 == y:
                result['up'].append(c)
                first_up_pos = min(first_up_pos, 10 * (i+1))
            elif 50 == y:
