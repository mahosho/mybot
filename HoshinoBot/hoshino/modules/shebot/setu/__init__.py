...
sv = Service('涩图', enable_on_default=False, manage_priv=Priv.SUPERUSER)
...
async def _(bot,ctx):
    ...
    user_priv = Service.get_user_priv(ctx)
    ...
    if COMMON_RE.match(message):
        if not g_is_online:
            ...
            if is_to_delete:
                #30秒后删除
                add_delay_job(my_delete_msg,delay_time=30,args=[bot,self_id,msg_id])
            return
        ...
        try:
            ...
            num = 1
        if Service.get_user_priv(ctx) < Priv.SUPERUSER:
            num = 1
